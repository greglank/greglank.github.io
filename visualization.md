...
- A table displaying the breakdown of wins and losses of every player against every other player

### Example Sheet

Other workbook sheets are more strategic in nature and can be used to inform future decision-making in the game. My favorite sheet displays a 2D coordinate plane that plots two variables against each other for each player: PFR% (preflop raise; the percentage of times the player raises a hand when given the opportunity) and VPIP% (voluntarily put money into pot; the percentage of times the player chooses to play a hand when given the opportunity). PFR% is the x (horizontal) axis, so players more to the right of the graph raise more hands. VPIP% is the y (vertical) axis, so players higher on the graph play more hands.

<img src="images/vpip-pfr.png" alt="Tableau screenshot" width="100%">
...
All of the graphs and stats in the Tableau workbook are interactive. You can mouse over the lines, click on the lines, and if there are tables, you can sort them in ascending or descending order.

Along the top there is a set of tabs you can click to see different sheets, such as the VPIP% vs. PFR% sheet described above.
...
Visuzalization is a handy tool to ~~establish bragging rights~~ both summarize what happened in the past (e.g., who won or lost over time) and provide strategic insights to inform future decision-making. This public Tableau workbook, accessible to everyone in my poker group, is a handy tool for fun and profit!
